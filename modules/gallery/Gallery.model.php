...
namespace candybox\Module\Gallery\Models;
...
class Gallery extends \candyCMS\Core\Models\Main {
  ...
  public function getId($iId, $bUpdate = false, $bAdvancedImageInformation = false) {
    ...
    if ($bUpdate)
      ...
      # Need to specify 'gallery' because this might be called for RSS feed generation
  ...
  public function createFile($sFile, $sExtension) {
    try {
      ...
      if (!ACTIVE_TEST && class_exists('\ImageMetadataParser') && \ImageMetadataParser::exifAvailable()) {
        $sLongFilename = PATH_UPLOAD . '/gallery/' . $this->_aRequest['id'] . '/original/' . $sFile;
