...
namespace candybox\Module\Search\Controllers;
 ...
 *
 */
class Search extends \candyCMS\Core\Controllers\Main {
  ...
  protected function _overview() {
    ...
    if (!isset($this->_aRequest[$this->_sController]['search']))
      ...
      if (!$this->oSmarty->isCached($oTemplate, UNIQUE_ID)) {
        $aSitemapModels = array_filter( array_map('trim', explode(',', DATA_SEARCH)));
        $aResults = array();
        ...
        $this->setTitle(I18n::get('search.title.show', $sString));
        $this->setDescription(I18n::get('search.description.show', $sString));
      }
